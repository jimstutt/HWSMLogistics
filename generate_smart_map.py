...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .list-item { background: #f8f9fa; padding: 10px; margin-bottom: 8px; border-radius: 4px; border-left: 4px solid #3498db; font-size: 0.9rem; }
        ...
        .modal-actions { display: flex; gap: 10px; margin-top: 15px; } .modal-actions button { flex: 1; }
...
<div id="mod-ship-add" class="modal">
    <div class="modal-content">
        ...
        <input type="text" id="s-dest" placeholder="Destination (e.g., Kampala, Uganda)">
...
function showCustomAdminAdd() {
    ...
    if (adminType === 'wh') { title = 'Add Warehouse'; fields = '<input type="text" id="aw-loc" placeholder="Location (e.g., Mogadishu)" style="'+s+'"><input type="number" id="aw-cap" placeholder="Capacity" style="'+s+'"><input type="text" id="aw-trans" placeholder="Transport" style="'+s+'"><input type="email" id="aw-email" placeholder="Email" style="'+s+'"><input type="text" id="aw-phone" placeholder="Phone" style="'+s+'">'; }
...
async function _mapCb() {
    ...
    // Initialize Geocoder for automatic city lookups
    ...
    try {
        ...
        for (const w of wh) {
            ...
            let pos = await geocode(w.location);
            if (!pos) pos = { lat: -1.29, lng: 36.82 }; // Fallback
            whCoords[wid] = pos;
        ...
        for (const s of sh) {
            let pos = await geocode(s.destination);
            if (!pos) {
                const basePos = whCoords[s.sourceWarehouse] || { lat: -1.29, lng: 36.82 };
                pos = { lat: basePos.lat + (Math.random() - 0.5) * 2, lng: basePos.lng + (Math.random() - 0.5) * 2 };
            }
...
print("✅ Pristine file overwritten. Smart Geocoding Map enabled.")
