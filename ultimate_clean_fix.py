...
# 3. The Master Script (Handles everything cleanly)
master_script = """
<script>
// --- TRACK ADMIN TAB ---
window.currentAdminTab = 'user';
const _origAdminTab = typeof adminTab === 'function' ? adminTab : function(){};
window.adminTab = function(t) {
    window.currentAdminTab = t; // Remember which tab is active
    _origAdminTab(t);
};
...
function showAdminAddModal() {
    const t = window.currentAdminTab;
    let title = 'Add Item';
    let fields = '';
...
print("✅ Hard reset complete. All fixes applied cleanly. Stray text is gone.")
